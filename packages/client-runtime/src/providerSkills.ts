...
// Fork: generic over the skill shape so local-agent skills (ServerLocalAgentSkill)
// can flow through the same helpers as provider skills.
export function dedupeProviderSkillsByName<T extends Pick<ServerProviderSkill, "name">>(
...
export function isProviderSkillUserInvocable(
  // Fork: local-agent skills carry no `userInvocable` flag, so it is optional here.
  skill: Pick<ServerProviderSkill, "enabled"> & { userInvocable?: boolean | undefined },
): boolean {
