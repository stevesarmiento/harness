...
describe("ThreadDeletionReactor drain", () => {
  ...
  effectIt.effect("waits for a published deletion the subscriber has not consumed yet", () =>
    Effect.gen(function* () {
      ...
      const checkpointDiffBlobs = {
        deleteByThreadId: () => Effect.void,
      } as unknown as CheckpointDiffBlobRepository["Service"];
      const layer = ThreadDeletionReactorLive.pipe(
        ...
        Layer.provide(Layer.succeed(CheckpointDiffBlobRepository, checkpointDiffBlobs)),
      );
