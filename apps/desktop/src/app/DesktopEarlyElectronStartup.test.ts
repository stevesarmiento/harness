...
describe("DesktopEarlyElectronStartup", () => {
  ...
  // Fork: implicit state lives under ~/.forma, not ~/.t3.
  it("keeps implicit development state under ~/.forma/dev when T3CODE_HOME is unset", () => {
    const preference = resolveEarlyLinuxPasswordStorePreference({
      env: {
        VITE_DEV_SERVER_URL: "http://127.0.0.1:5173",
      },
      ...
      joinPath,
      readFileString: (path) => {
        assert.equal(path, "/home/user/.forma/dev/desktop-settings.json");
