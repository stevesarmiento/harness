...
export function useInlinePanelWidth(options?: {
  ...
  // Fork: hoisted callers have no host element to measure, so clamp against
  // the viewport only (enabled=false skips the container observer).
  const hostRef = useRef<HTMLDivElement | null>(null);
  const maxWidth = useClampedMaxWidth(hostRef, false);
...
export function PreviewPanelShell(props: {
  ...
  maximized?: boolean;
  // Fork: parent-owned width state so width-synced chrome stays aligned.
  ...
  const internalResizable = useResizableWidth({
  ...
  // Fork: the parent may own the width state (useInlinePanelWidth) when it
  // also renders width-synced chrome above the panel.
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Fork: no border-l when maximized — the chat column collapses to zero
        // width, so the divider would paint as a stray line at the card's left edge.
