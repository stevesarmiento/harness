...
// Fork: componentPreviewTargets + provider display helpers retained alongside upstream provider-instance helpers
import { openComponentPreviewSurface } from "../componentPreviewTargets";
import { registerFaviconProjectForThread } from "~/browserFaviconStore";
import { getProviderDisplayName, getProviderModelCapabilities } from "../providerModels";
import {
...
// Fork: RightPanelToggleControl retained; upstream expandedImageKey ported
import {
  PanelLayoutControls,
...
function ChatViewContent(props: ChatViewProps) {
  ...
  // Fork: imperative handle for the Forma git-actions control
  ...
  // Fork: base interaction mode feeds the "ask" override layered over upstream's plan gating
  ...
  // Fork: shared with the chrome header's tab-strip zone so it stays width-synced
  ...
  // Fork: "ask" interaction mode override from thread extensions; the effective
  // interaction mode is derived below once upstream's plan-mode gating inputs exist.
  const askInteractionModeActive =
    threadExtensionState?.interactionModeOverride === "ask" || baseInteractionMode === "ask";
  ...
  const { enabled: interactionModeEnabled, interactionMode: standardInteractionMode } =
    resolveComposerInteractionMode({
      planModeEnabled: settings.planModeEnabled,
      provider: activeProviderStatus,
      interactionMode: baseInteractionMode === "ask" ? "default" : baseInteractionMode,
    });
  // Fork: layer the "ask" mode over upstream's plan-mode gating
  const interactionMode: FormaInteractionMode = askInteractionModeActive
    ? "ask"
    : standardInteractionMode;
  ...
  // Fork: upstream's composer context strip is replaced by the Forma ComposerMetaBar.
  ...
  const handleInteractionModeChange = useCallback(
    // Fork: accepts the fork's wider FormaInteractionMode (includes "ask")
  ...
  const onSend = async (
    ...
    const {
      ...
      // Fork: the send path uses standardInteractionMode + askOverride below
    ...
    // Fork: expand project-local agent commands into the outgoing prompt
    ...
    // Fork: turns sent while the provider is busy join the composer queue
    ...
    // Fork: queued turns remain in the composer queue until the server promotes
    // them into normal history, so skip anchoring and the optimistic append.
    if (!shouldQueueTurn) {
    ...
    if (failure === null && turnAttachmentsResult._tag === "Success") {
      ...
      // Fork: clear the preparing-worktree indicator once dispatching, and
      // route busy-thread sends into the composer queue (askOverride carries
      // the fork's "ask" interaction mode).
      beginLocalDispatch({
  ...
  const rightPanelContent = activeThreadRef ? (
    ...
    ) : renderedRightPanelSurface?.kind === "componentPreview" ? (
      // Fork: component preview surface
      <Suspense fallback={null}>
  ...
  return (
    <div className="relative flex min-h-0 min-w-0 flex-1 flex-col">
      {/* Fork: Forma workspace chrome — top bar sits on the window chrome above the inset card */}
      <header
        data-chat-header
        className={cn(
            ...
            >
              ...
              <div className="relative flex min-h-0 flex-1 flex-col">
                ...
                {showScrollToBottom && (
                  ...
                  >
                    {/* Fork: Forma glass pill styling; upstream restore-on-return behavior */}
                    <button
                      type="button"
              ...
              >
                ...
                >
                  <div className="group/composer-stack pointer-events-auto relative z-10">
                    ...
                    >
                      {/* Fork: Forma composer column (max-w + meta bar) instead of upstream's ComposerSurface shell */}
