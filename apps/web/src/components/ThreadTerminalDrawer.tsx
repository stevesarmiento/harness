...
import {
  // Fork: symbols-react icons instead of lucide.
  IconPlus as Plus,
  ...
  return (
    ...
    >
      ...
      <div className="min-h-0 w-full flex-1">
        ...
        >
          {/* Fork: sidebar renders on the left, so content takes order-2. */}
          <div className={cn("min-w-0 flex-1", hasTerminalSidebar && "order-2")}>
            {isSplitView ? (
              <div
                  ...
                  return (
                    <div key={terminalGroup.id} className="pb-0.5">
                      {showGroupHeaders && (
                        <button
                          type="button"
                          // Fork: text-ui-2xs uppercase heading in place of upstream's text-[11px].
                          className={`text-ui-2xs flex h-[22px] w-full cursor-pointer items-center gap-1 rounded px-1.5 uppercase tracking-[0.08em] ${
                            isGroupActive
                        ...
                        {terminalGroup.terminalIds.map((terminalId) => {
                          ...
                          return (
                            <div
                              key={terminalId}
                              className={cn(
                                // Fork: text-code-compact in place of upstream's text-xs.
                                "text-code-compact group/tab flex h-6 w-full items-center gap-0.5 rounded-md pr-2 pl-1.5",
                                isActive
                                  ? "bg-accent text-foreground"
                                  : "text-muted-foreground hover:bg-accent/60 hover:text-foreground",
                              )}
                            >
                              {/* Fork: group indent glyph. */}
                              {showGroupHeaders && (
