...
/**
 * Fork: rows below are shared between the legacy upstream panels (General /
 * Appearance, kept reachable at their old URLs) and the Forma settings
 * sections, so each upstream setting renders from a single source.
 */
export function AppearanceContrastSettingsRow() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
...
export function PanelAnimationsSettingsRow() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
...
export function ProactivePanelsSettingsRow() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
...
export function ContinueThreadsAfterServerUpdateSettingsRow() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
...
export function UnpinConfirmationSettingsRow() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
...
/** Renders nothing outside the desktop app, where the quit shortcut does not exist. */
export function QuitConfirmationSettingsRow() {
  const settings = usePrimarySettings();
  const updateSettings = useUpdatePrimarySettings();
      ...
      resetAction={
        settings.confirmQuit !== DEFAULT_UNIFIED_SETTINGS.confirmQuit ? (
          <SettingResetButton
            ...
            onClick={() => updateSettings({ confirmQuit: DEFAULT_UNIFIED_SETTINGS.confirmQuit })}
          />
        ) : null
      }
      control={
        ...
        >
          <SelectTrigger size="sm" className="w-full sm:w-40" aria-label="Quit shortcut behavior">
