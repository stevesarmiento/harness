...
// Fork: subtle copy-button styling + micro-fade motion tokens.
...
// Fork: user messages group with their following content rows so each user
// message can stick within its own section.
...
// Fork: hairline header — sticky user sections carry their own padding.
...
// Fork: backdrop treatments for the sticky user-message card.
...
export const MessagesTimeline = memo(function MessagesTimeline({
  ...
  // Fork: the citation hook pins raw row ids, but the list renders grouped
  // items — map each pinned row id to the render item that contains it.
  const groupedCitationAlwaysRender = useMemo(() => {
    if (!citationAlwaysRender) {
      return undefined;
    }
    const keys = citationAlwaysRender.keys.map((key) => {
      for (const item of renderedItems) {
        if (item.kind === "standalone-row") {
          if (item.row.id === key) {
            return item.id;
          }
        } else if (item.userRow.id === key || item.contentRows.some((row) => row.id === key)) {
          return item.id;
        }
      }
      return key;
    ...
    return { keys };
  }, [citationAlwaysRender, renderedItems]);
  ...
  const anchoredEndSpace = useMemo(() => {
    // Fork: anchors resolve against grouped render items; user messages are
    // always section heads, matching upstream's user-role-only anchoring.
    const config = resolveChatListAnchoredEndSpace(
      renderedItems,
      anchorMessageId,
      (item) => (item.kind === "user-section" ? item.userRow.message.id : null),
      ...
      <TimelineRowActivityCtx value={activityState}>
        ...
        >
          ...
          <LegendList<TimelineRenderItem>
            ref={listRef}
            data={renderedItems}
            extraData={renderedItems.length}
            ...
            {...(groupedCitationAlwaysRender ? { alwaysRender: groupedCitationAlwaysRender } : {})}
...
function UserTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "message" }> }) {
  ...
  return (
    // Fork: full-width bordered user-message card instead of chat bubble.
    <div className="group flex flex-col items-end gap-1 px-0.5">
      ...
      >
        <div className="flex shrink-0 items-center gap-2">
          <Tooltip>
            <TooltipTrigger
              // Fork: Forma micro type scale for message timestamps.
...
function AssistantMessageMeta({
  ...
  return (
    // Fork: timestamp stays faintly visible; only the copy button fades in.
...
function WorkingTimelineRow({ row }: { row: Extract<TimelineRow, { kind: "working" }> }) {
  ...
  return (
    // Fork: PixelGridLoader working indicator with Forma micro type scale.
