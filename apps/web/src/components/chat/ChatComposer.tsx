...
// Fork: provider interaction-mode toggle support
...
export interface ChatComposerHandle {
  ...
  getSendContext: () => {
    ...
    // Fork: tri-state Forma interaction mode
...
export interface ChatComposerProps {
  ...
  // Fork: project-local agent skills/commands
...
export const ChatComposer = memo(function ChatComposer(props: ChatComposerProps) {
  ...
  const selectedProviderSlashCommands = selectedProviderStatus
    ...
    : [];
  // Fork: project-local agent skills join provider skills in composer searches.
  const allComposerSkills = [...localAgentInventory.skills, ...selectedProviderSkills];
  ...
  // Fork: tri-state Forma interaction mode (default/ask/plan). Plan mode is a
  // legacy feature behind Settings -> Beta; with the flag off ChatView forces
  // the effective mode to "default", so hiding the toggle can't trap anyone
  // in plan mode.
  const planModeUiEnabled =
    settings.planModeEnabled &&
    getProviderInteractionModeToggle(providerStatuses, selectedProvider);
  const interactionMode: FormaInteractionMode = planModeUiEnabled
  ...
  const supportedInteractionModes = useMemo<ReadonlyArray<ServerProviderSupportedInteractionMode>>(
    () =>
      ...
      (planModeUiEnabled ? (["default", "plan"] as const) : (["default"] as const)),
  ...
  const composerMenuItems = useMemo<ComposerCommandItem[]>(() => {
    ...
    if (composerTrigger.kind === "slash-command") {
      ...
      // Fork: project-local agent commands appear alongside provider commands.
    ...
    if (composerTrigger.kind === "skill") {
      // Fork: local-agent skills surface as project skills in the $ menu.
  ...
  const onSelectComposerItem = useCallback(
    (item: ComposerCommandItem) => {
      ...
      if (item.type === "slash-command") {
        ...
        // Fork: tri-state interaction modes include "ask".
  ...
  const submitComposer = useCallback(
    (event?: { preventDefault: () => void }, intent: ComposerSubmissionIntent = "foreground") => {
      // Fork: pending-question submits bypass ordinary send gates.
      if (
  ...
  const restingBlockDefs = [
    ...(providerTraitsPicker
      ...
      : []),
    // Fork: Forma interaction-mode pill replaces the runtime/mode selects;
    // runtime mode lives in the ComposerMetaBar.
    ...(planModeUiEnabled
      ? [
          {
            id: "mode",
  ...
  return (
    ...
    >
      {composerControlsInStrip && restingControlsHost
        ...
        : null}
      {/* Fork: queued turns panel for the send queue */}
